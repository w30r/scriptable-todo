// Scriptable Todo Master - Widget + Full Interactive UI
...
const todoId = args.queryParameters?.id;
const todoAction = args.queryParameters?.todoAction;
...
function createWidget(todos, scriptName) {
  ...
  widget.url = `scriptable://run/${scriptName}?action=ui`;

  return widget;
}

function createSmallWidget(todos, scriptName) {
  ...
  w.url = `scriptable://run/${scriptName}?action=ui`;

  return w;
}

async function runWidgetMode() {
  const scriptName = "Todo Master";
  const manager = new TodoManager();
  await manager.fetchTodos();

  if (widgetFamily === "small") {
    const widget = createSmallWidget(manager.todos, scriptName);
...
async function showInteractiveUI() {
  const manager = new TodoManager();
  await manager.fetchTodos();
  ...
  while (true) {
    ...
    if (pressed === manager.todos.length) {
    ...
    detailAlert.addCancelAction("Back");
...
async function handleQuickAction() {
  ...
  if (todoAction === "toggle" && todoId) {
    await manager.toggleTodo(todoId);
  } else if (todoAction === "delete" && todoId) {
    await manager.deleteTodo(todoId);
  }
