...
router.get('/', async (req, res) => {
  try {
    const cards = await ElsaContext.find().sort({ createdAt: -1 });
    res.json(cards);
  } catch (err) {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const card = await ElsaContext.findByIdAndUpdate(
      req.params.id,
      updates,
...
router.delete('/:id', async (req, res) => {
  try {
    const card = await ElsaContext.findByIdAndDelete(req.params.id);
    if (!card) return res.status(404).json({ error: 'Card not found' });
